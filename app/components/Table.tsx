...
export function Table(props: TableProps & { className: string }) {
  return (
    <ResizableTableContainer
      className={twMerge(
        "h-auto w-full overflow-hidden relative border dark:border-zinc-600",
        props.className
      )}
    >
...
const resizerStyles = tv({
  extend: focusRing,
  base: "w-px px-[8px] translate-x-[8px] box-content py-1 h-5 bg-clip-content bg-gray-400 dark:bg-zinc-500 forced-colors:bg-[ButtonBorder] cursor-col-resize resizing:bg-blue-600 forced-colors:resizing:bg-[Highlight] resizing:w-[2px] resizing:pl-[7px] -outline-offset-2",
});
...
export function TableHeader<T extends object>(props: TableHeaderProps<T>) {
  ...
  return (
    <AriaTableHeader
      ...
      className={composeTailwindRenderProps(
        props.className,
        "sticky top-0 z-10 bg-gray-100/60 dark:bg-zinc-700/60 backdrop-blur-md supports-[-moz-appearance:none]:bg-gray-100 dark:supports-[-moz-appearance:none]:bg-zinc-700 forced-colors:bg-[Canvas] border-b dark:border-b-zinc-700"
      )}
    >
