...
const RenderPage = ({params} :PageProps) => {
    ...
    useEffect(()=>{
        const fetching = async()=>{
            ...
            setarchiveTitle(result.data.title);
            setarchiveDescription(result.data.description);
            setIsloading(false);
        }
        fetching();
    },[])
    ...
    <div className="space-y-4 pb-6 pt-7">
      ...
      {isLoading ? (
        ...
        <div className='items-center flex justify-center'>
          <p className='text-red-600'>No videos added yet.</p>
        </div>
      )}
    </div>
  );
